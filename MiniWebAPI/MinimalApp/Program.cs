...
app.MapPost("/create", ([FromBody]CoffeeModel coffee,[FromServices] ICoffeeService service) => 
{
...
app.MapPut("/update", (CoffeeModel newCoffee, ICoffeeService service) =>
{
    ...
    if (updatedCoffee is null)
        Results.NotFound("Coffee not found");
...
app.MapDelete("/delete", (int id, ICoffeeService service) =>
{
    ...
    if (!result)
        Results.NotFound("Something went wrong");
